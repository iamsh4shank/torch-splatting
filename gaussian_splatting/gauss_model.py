...
from gaussian_splatting.utils.gauss_utils import depth_to_3d
...
class GaussModel(nn.Module):
    ...
    * Attributes
    ...
    def create_from_depth_confidence(self, depth_map, confidence_map, intrinsics):
        points_3d = depth_to_3d(depth_map, intrinsics)

        fused_point_cloud = points_3d.view(-1, 3).float().cuda()
        fused_opacity = confidence_map.view(-1, 1).float().cuda()
        ...
        dist2 = torch.clamp_min(torch.sum(fused_point_cloud**2, dim=1, keepdim=True), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1 

        if self.debug:
            fused_opacity = inverse_sigmoid(0.9 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        ...
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = nn.Parameter(fused_opacity.requires_grad_(True))
        self.max_radii2D = torch.zeros((self._xyz.shape[0]), device="cuda")
        return self
